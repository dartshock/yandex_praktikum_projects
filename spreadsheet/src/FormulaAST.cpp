...
public:
    ...
    virtual double Evaluate(
        const std::function<double(Position pos)>& get_cell_value) const = 0;
    ...
    void PrintFormula(std::ostream& out, ExprPrecedence parent_precedence,
        ...
        bool parens_needed =
            PRECEDENCE_RULES[parent_precedence][precedence] & mask;
        if (parens_needed) {
            out << '(';
        }
...
public:
    ...
    double Evaluate(const std::function<double(Position pos)>& get_cell_value)
        const override {
...
public:
    ...
    double Evaluate(const std::function<double(Position pos)>& get_cell_value)
        const override {
        double value = operand_->Evaluate(get_cell_value);
...
public:
    ...
    double Evaluate([[maybe_unused]] const std::function<double(Position pos)>&
                        get_cell_value) const override {
        return value_;
    }
...
double FormulaAST::Execute(
    const std::function<double(Position pos)>& get_cell_value) const {
